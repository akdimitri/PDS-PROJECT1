...
int main(int argc, char **argv) {
	...
	if (argc == 3) {
		...
		double CILK_PARALLEL_TIMES[10][15][8];
		...
		for(k = 0; k < 10; k++){
			...
			for( i = 12; i <= 26; i++){
				...
				for( j = 1; j <= 8; j++){
					...
					/* print sorted vector */
					CILK_PARALLEL_TIMES[k][i-12][j-1] = qsort_time;
		...
		write_results_to_file( SEQUENTIAL_TIMES, DEFAULT_SEQUENTIAL_TIME, CILK_PARALLEL_TIMES);
	}
	else{
